...
async function checkoutEvidenceItem(evidenceItemId, handler, organization) {
    ...
    try {
        let result = await contract.methods.checkoutEvidenceItem(evidenceItemId).send({ 
            from: account,
...
checkoutEvidenceItem(evidenceItemId, handler, organization).then(() => process.exit(0));
